...
-(AVPlayerItem *)getPlayItem:(int)videoIndex{
    NSString *VideoFile = [NSString stringWithFormat:@"%i.mp4",videoIndex];
    NSString *urlStr=[[NSBundle mainBundle]pathForResource:VideoFile ofType:nil];
...
-(void)playbackFinished:(NSNotification *)notification{
    NSLog(@"视频播放完成.");
    _nowIndex = (_nowIndex + 1) % 3;
    [self changeIndex:_nowIndex];
}
...
-(void)addProgressObserver{
    ...
    UIProgressView *progress=self.progress;
    //这里设置每秒执行一次
    [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 500.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
...
- (IBAction)navigationButtonClick:(UIButton *)sender {
    [self removeNotification];
    [self removeObserverFromPlayerItem:self.player.currentItem];
    [self changeIndex:(int)sender.tag];
}
